...
`web/production-run.html` is a separate guarded path for mainnet production artifacts generated by `real:token-account:build`, `real:fund:build`, and `real:lifecycle:build`.
...
The command prints a `production_submitter_url` on `127.0.0.1`, redirects `/` to `/web/production-run.html`, and serves only safe static paths. It does not read `.env*`, does not serve `.tascverifier/`, does not expose package metadata, and does not call RPC or send transactions.

It does not build new transactions, accept private keys, or persist the RPC URL. The operator pastes or selects a `tasc.production_token_account_setup_transaction`, `tasc.production_fund_transaction`, or `tasc.production_lifecycle_transaction` artifact, and the browser core accepts only `solana-mainnet-beta` / `mainnet` artifacts for the exact 10 USDC production goal. Before a send, the page requires the connected wallet address to match the artifact signer and requires an explicit production-send checkbox.

After submission, the page polls `getSignatureStatuses` through the operator-provided RPC URL and prints the matching next command. Setup artifacts point back to `npm run real:preflight -- --env .env.solana-mainnet.local`; fund, claim, attest, and release artifacts print the matching `npm run real:capture:record -- --transaction <artifact> --signature <sig>` command. Claim and release artifacts include the timer placeholders required by the production capture recorder.
...
- the production artifact runner validates mainnet-only fund/release artifacts and emits matching capture commands
