...
`web/production-run.html` is a separate guarded path for mainnet production artifacts generated by `real:fund:build` and `real:lifecycle:build`.
...
It does not build new transactions, accept private keys, or persist the RPC URL. The operator pastes or selects a `tasc.production_fund_transaction` or `tasc.production_lifecycle_transaction` artifact, and the browser core accepts only `solana-mainnet-beta` / `mainnet` artifacts for the exact 10 USDC production amount. Before a send, the page requires the connected wallet address to match the artifact signer and requires an explicit production-send checkbox.
