...
- `npm run real:env:init:plan`, `npm run real:env:init`, `npm run real:env:plan`, `npm run real:env:validate`, and `.env.example` provide the safe ignored mainnet env setup path, chmod the private env file to `0600`, preserve existing values unless forced, reject private-key-like entries and devnet/test/local/example RPC hosts, and report the public values needed before preflight
