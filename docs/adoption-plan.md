...
- `npm run real:env:plan`, `npm run real:env:validate`, and `.env.example` provide the safe ignored mainnet env setup path, reject private-key-like entries and devnet/test/local/example RPC hosts, and report the public values needed before preflight
...
- use `real:env` as the default mainnet public configuration doctor before preflight
