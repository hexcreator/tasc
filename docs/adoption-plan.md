...
- `npm run real:readiness:plan` shows the remaining evidence needed before the actual `$10 in less than a minute` goal can be claimed, and `npm run real:readiness` rejects devnet/example evidence or production JSON without live mainnet RPC verification as not real-money ready
...
- use `examples/private-beta/production-payout-evidence.example.json` as the schema for the first mainnet USDC payout artifact; do not count the goal complete until `real:readiness` returns `ready_for_goal: true` with non-example evidence, a mainnet RPC URL, and an expected genesis hash
