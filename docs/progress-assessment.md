...
| Private beta product | 77% | Protocol mechanics are live-proven, packaged behind a one-command devnet proof runner, and visible in a static browser operator console with feed import, signed task inputs, worker submission capture, direct verifier API submission, persisted verifier ingestion, Solana attest-field handoff, a dependencyless verifier API with bearer auth, durable artifacts, and persistent duplicate ledger, Solana wallet connection, live task-account refresh, role detection, next-action readiness, guarded wallet transaction construction/submission for claim, attest, release, refund, and timeout-refund, automated mock-provider validation of both wallet submission transports, and a local private-beta launcher that serves the static app plus verifier API together with same-origin verifier auto-fill. Missing live wallet-extension QA, hosted fresh inventory, deployed verifier operations, reputation, disputes, and durable marketplace metadata. |
