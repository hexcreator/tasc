...
- A dependencyless local private-beta launcher can serve the static app and verifier API together on localhost, generate an ephemeral bearer token when needed, restrict static file exposure to public app/artifact paths, validate verifier auth, ingest the bundled worker proof, and write local verifier ledger/artifacts.
