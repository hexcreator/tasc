...
| Private beta product | 74% | Protocol mechanics are live-proven, packaged behind a one-command devnet proof runner, and visible in a static browser operator console with feed import, signed task inputs, worker submission capture, direct verifier API submission, persisted verifier ingestion, Solana attest-field handoff, a dependencyless verifier API with bearer auth, durable artifacts, and persistent duplicate ledger, Solana wallet connection, live task-account refresh, role detection, next-action readiness, guarded wallet transaction construction/submission for claim, attest, release, refund, and timeout-refund, and automated mock-provider validation of both wallet submission transports. Missing live wallet-extension QA, hosted fresh inventory, deployed verifier operations, reputation, disputes, and durable marketplace metadata. |
...
If Solana is the preferred path, the next increment is now concrete: run the guarded wallet-send flow against Phantom or Solflare in a normal extension browser, deploy the verifier API with real secret handling, and publish fresh proof indexes as a hosted feed instead of manually pasting artifacts.
