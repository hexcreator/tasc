...
| Private beta product | 57% | Protocol mechanics are live-proven, packaged behind a one-command devnet proof runner, and visible in a static browser operator console with feed import, signed task inputs, worker submission capture, Solana wallet connection, live task-account refresh, role detection, next-action readiness, and guarded wallet transaction construction/submission for claim, attest, release, refund, and timeout-refund. Missing live wallet QA, hosted fresh inventory, verifier service ingestion, reputation, disputes, and durable metadata. |
...
4. Product surfaces: buyer task creation, hosted fresh inventory publishing, live wallet QA, verifier-service proof ingestion, notifications, and support tooling.
...
If Solana is the preferred path, the next increment is now concrete: live-test guarded wallet sends in a normal extension environment, then wire captured submission proofs into a verifier service and publish fresh proof indexes as a hosted feed instead of manually pasting artifacts.
