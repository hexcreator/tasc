...
| Private beta product | 45% | Protocol mechanics are live-proven, packaged behind a one-command devnet proof runner, and visible in a static browser operator console with Solana wallet connection, live task-account refresh, role detection, next-action readiness, and guarded wallet transaction construction/submission for claim, attest, release, refund, and timeout-refund. Missing live wallet QA, real task inventory, verifier service, reputation, disputes, and durable metadata. |
...
4. Product surfaces: buyer task creation, proof-bundle inventory import, live wallet QA, notifications, and support tooling.
...
If Solana is the preferred path, the next increment is now concrete: live-test guarded browser wallet sends in a normal extension environment, then make proof bundles and fresh task inventory loadable from the static feed.
