...
| Private beta product | 53% | Protocol mechanics are live-proven, packaged behind a one-command devnet proof runner, and visible in a static browser operator console with feed import, signed task inputs, Solana wallet connection, live task-account refresh, role detection, next-action readiness, and guarded wallet transaction construction/submission for claim, attest, release, refund, and timeout-refund. Missing live wallet QA, signed worker submission capture, hosted fresh inventory, verifier service, reputation, disputes, and durable metadata. |
...
4. Product surfaces: buyer task creation, signed worker submission capture, hosted fresh inventory publishing, live wallet QA, notifications, and support tooling.
...
If Solana is the preferred path, the next increment is now concrete: capture signed worker submissions in the browser, live-test guarded wallet sends in a normal extension environment, then publish fresh proof indexes as a hosted feed instead of manually pasting artifacts.
