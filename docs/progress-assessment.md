...
| Testnet MVP | 90% | Base Sepolia is blocked by faucet friction; Solana devnet now has a finalized lifecycle-enabled program deploy, finalized account-state fund transaction, live account scan, claimable index admission, finalized SPL mint/token-account setup, decoded live SPL account evidence, a guarded token-backed funding transaction admitted with live vault custody, and confirmed live claim/attest transactions. The remaining testnet gap is SPL payout/refund CPI and post-release/refund scanner evidence. |
...
That is probably 1 serious engineering increment away in this prototype on Solana: implement SPL release/refund CPI and scan the completed payout lifecycle. A production system that can safely support a global market for instant `$10` work is much farther: likely months of focused buildout after the testnet loop works, because the hard parts become abuse resistance, demand liquidity, dispute quality, and reliable user experience.

If Solana is the preferred path, the next increment is now concrete: add program-signed token release/refund, then scan the completed lifecycle into the same indexer boundary.
